...
const message = {
    ...
    setting: {
        ...
        title: 'パネルエイリアス',
        panelPort: 'パネルポート',
        titleHelper: '特別なキャラクターの開始、英語、中国語、数字、スペース、.-および_、長さ3-30をサポートします',
        portHelper:
        ...
        ap_chengdu: '成都',
        ap_chongqing: '唐辛子',
        ap_shenzhen_fsi: '深Shenzhen Financial',
        ...
        scStandard:
            '標準ストレージは、リアルタイムアクセス、頻繁なデータ相互作用などを必要とする多数のホットファイルを備えたビジネスシナリオに適しています。',
        scStandard_IA:
            '低周波ストレージは、アクセス頻度が比較的低いビジネスシナリオに適しており、少なくとも30日間データを保存します。',
        scArchive: 'アーカイブストレージは、アクセス頻度が非常に低いビジネスシナリオに適しています。',
        scDeep_Archive: '耐久性のあるコールドストレージは、アクセス頻度が非常に低いビジネスシナリオに適しています。',
        archiveHelper:
            'アーカイブストレージファイルは直接ダウンロードすることはできず、最初に対応するクラウドサービスプロバイダーのWebサイトから復元する必要があります。注意して使用してください！',
        backupAlert:
            'クラウドプロバイダーがS3プロトコルと互換性がある場合、Amazon S3をバックアップに直接使用できます。',
        domain: 'ドメインを加速します',
        ...
        allowIPEgs: '1行に1つ。たとえば、 n172.16.10.111  n172.16.10.0/24',
        mfa: '二因子認証（2FA）',
        ...
        noAuthSetting: '不正な設定',
        noAuthSettingHelper: `whenUsersDon'指定されたセキュリティの入り口でログイン、またはドン'tAccessThePanelFromSpecifiedIpOrDomainName,ThisResponseCanHidPanelCharacteristic`,
